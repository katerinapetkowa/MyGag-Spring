	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-lg-8">
				...
				<div style="width: 50px;">
					<div style="float: left; width: 5px">
						<form action="postlike" method="post">
							...
							<input id="author" name="post_id" type="hidden"
								...
								required> <input id="username" name="username"
								type="hidden"
								value="<c:out value="${sessionScope.loggedAs}"></c:out>"
								size="30" required>
							<c:choose>
								<c:when
									test='${PostsManager.getInstance().validUpvote(post.postId, sessionScope.loggedAs)}'>
									<input type="image" id="image" src="img/redheart2.png"
										onclick="action();" alt="Submit" width="38" height="38">
								</c:when>
								<c:otherwise>
									<input type="image" id="image" src="img/heart.png"
										onclick="action();" alt="Submit" width="38" height="38">
								</c:otherwise>
							</c:choose>
						</form>
					...
					<div style="float: right; width: 5px">
						<form action="postdislike" method="post">
							...
							<c:choose>
								<c:when
									test='${PostsManager.getInstance().validDownvote(post.postId, sessionScope.loggedAs)}'>
									<input type="image" id="image2" src="img/reddislike2.png"
										onclick="action2();" alt="Submit" width="38" height="38">
								</c:when>
								<c:otherwise>
									<input type="image" id="image2" src="img/dislikebutton.png"
										onclick="action2();" alt="Submit" width="38" height="38">
								</c:otherwise>
							</c:choose>
					...
					<c:when test="${sessionScope.loggedAs == post.username}">
						<form action="deletePost" method="post">
							<button data-toggle="tooltip" data-placement="top"
								title="Delete post"
								style="float: right; background: none !important; border: none; padding: 0 !important; font: inherit; border-bottom: none; cursor: pointer"
								onmouseover="this.style.color = '#808080'"
								onmouseout="this.style.color = '#222222'">delete</button>
							<input id="author" name="post_id" type="hidden"
								value="<c:out value= "${post.postId}"></c:out>" size="30"
								required> <input id="username" name="username"
								type="hidden"
				...
				<br> <br> <a style="color: gray" href=""> <c:out
								...
								<c:when test="${sessionScope.loggedAs == comment.username}">
									<form action="deleteComment" method="post">
										<input id="username" name="username" type="hidden"
											value="<c:out value="${sessionScope.loggedAs}"></c:out>"
											size="30" required> <input id="author" name="post_id"
											type="hidden" value="<c:out value= "${post.postId}"></c:out>"
											size="30" required> <input id="comments"
											name="comment_id" type="hidden"
